<template>
  <ul class="tasks__view">
    <li v-for="item in tasksList" :key="item.id" class="task__item">
      <input class="custom-checkbox" type="checkbox" v-if="getStatusWindow.actTaskShow" :id="item.id" :value="item.text"  @change="checkedTask({id:item.id, where:'completed', whereFrom:'active'})" >
      <label :for="item.id"></label>
      <span>{{item.text}}</span>
      <button class="btn-del" @click="checkedTask({id:item.id, where:'deleted', whereFrom:'completed'})" v-if="getStatusWindow.compTaskShow">
